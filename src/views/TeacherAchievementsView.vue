...
import { MOCK_TEACHER_ACHIEVEMENTS } from '@/services/authStore'
...
const filters = ref({ category: '', status: '', source: '', search: '' })

const newAchievement = ref({
  title: '', type: 'education', category: 'Результативность учебной деятельности',
  description: '', level: 'вузовский', date: new Date().toISOString().split('T')[0], skills: ''
})

const categories = ['Результативность учебной деятельности', 'Развитие способностей обучающихся', 'Личный вклад', 'Общественное признание', 'Профессиональное сообщество', 'Дополнительное образование', 'Работа в ЦДО']
const types = [
  { value: 'science', label: '🔬 Научная' }, { value: 'education', label: '📚 Учебная' },
  { value: 'social', label: '🤝 Социальная' }, { value: 'methodical', label: '📋 Методическая' }
]
const levels = ['вузовский', 'региональный', 'всероссийский', 'международный']

onMounted(() => {
  setTimeout(() => { achievements.value = MOCK_TEACHER_ACHIEVEMENTS; isLoading.value = false }, 300)
})
...
function clearFilters() { filters.value = { category: '', status: '', source: '', search: '' } }

function addAchievement() {
  const achievement = {
    id: Date.now(), title: newAchievement.value.title, type: newAchievement.value.type,
    category: newAchievement.value.category, source: 'manual', status: 'pending',
    date: newAchievement.value.date, description: newAchievement.value.description,
    files: [], skills: newAchievement.value.skills.split(',').map(s => s.trim()).filter(Boolean),
    level: newAchievement.value.level
  }
  achievements.value.unshift(achievement)
  showAddModal.value = false
  newAchievement.value = { title: '', type: 'education', category: 'Результативность учебной деятельности', description: '', level: 'вузовский', date: new Date().toISOString().split('T')[0], skills: '' }
}

function deleteAchievement(id) { achievements.value = achievements.value.filter(a => a.id !== id) }
function getStatusLabel(s) { return { approved: 'Подтверждено', pending: 'На проверке', rejected: 'Отклонено' }[s] || s }
function getLevelLabel(l) { return { вузовский: 'Вуз', региональный: 'Регион', всероссийский: 'РФ', международный: 'Мир' }[l] || l }

const activeFiltersCount = computed(() => {
  let c = 0; if (filters.value.category) c++; if (filters.value.status) c++; if (filters.value.source) c++; if (filters.value.search) c++; return c
})
</script>
  ...
  <div class="achievements-page">
    <div class="page-header">
      ...
      <div class="header-actions">
        <button class="btn btn-secondary" @click="showFilter = !showFilter">
          ...
          Фильтр <span v-if="activeFiltersCount" class="filter-badge">{{ activeFiltersCount }}</span>
        </button>
    ...
    <div v-if="showFilter" class="filter-panel">
      <div class="filter-row">
        <div class="filter-group"><label>Поиск</label><input v-model="filters.search" type="text" placeholder="Поиск..." class="filter-input" /></div>
        <div class="filter-group"><label>Категория</label><select v-model="filters.category" class="filter-select"><option value="">Все</option><option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option></select></div>
        <div class="filter-group"><label>Статус</label><select v-model="filters.status" class="filter-select"><option value="">Все</option><option value="approved">Подтверждено</option><option value="pending">На проверке</option></select></div>
        <div class="filter-group"><label>Источник</label><select v-model="filters.source" class="filter-select"><option value="">Все</option><option value="manual">Ручной ввод</option><option value="moodle">Moodle</option><option value="cdo">ЦДО</option></select></div>
        <button class="btn btn-ghost" @click="clearFilters">Сбросить</button>
      </div>
    </div>

    <div v-if="isLoading" class="loading-state"><div class="spinner"></div><p>Загрузка достижений...</p></div>

    <div v-else class="achievements-grid">
      <div v-for="item in filteredAchievements" :key="item.id" class="achievement-card" @click="selectedAchievement = item">
        <div class="card-type-icon">{{ { science: '🔬', education: '📚', social: '🤝', methodical: '📋' }[item.type] || '📌' }}</div>
        <div class="card-content">
          ...
          <div class="card-skills"><span v-for="skill in item.skills" :key="skill" class="skill-chip">{{ skill }}</span></div>
        </div>
        <div class="card-status">
          <span :class="'status-badge status-' + item.status">{{ getStatusLabel(item.status) }}</span>
          <span v-if="item.source === 'moodle'" class="source-badge moodle">Moodle</span>
          <span v-if="item.source === 'cdo'" class="source-badge cdo">ЦДО</span>
        </div>
      ...
      <div v-if="filteredAchievements.length === 0" class="empty-state">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="1.5"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5C7 4 7 7 7 7v.5" /><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5C17 4 17 7 17 7v.5" /><path d="M12 22v-8" /><path d="M10 14h4v4h-4Z" /><path d="M8 9h8l-1 7H9Z" /></svg>
    ...
    <div v-if="showAddModal" class="modal-overlay" @click.self="showAddModal = false">
      <div class="modal">
        <div class="modal-header"><h2>Новое достижение</h2><button class="modal-close" @click="showAddModal = false">&times;</button></div>
        <form @submit.prevent="addAchievement" class="modal-body">
          ...
          <div class="form-row">
            <div class="form-group"><label>Тип</label><select v-model="newAchievement.type"><option v-for="t in types" :key="t.value" :value="t.value">{{ t.label }}</option></select></div>
            <div class="form-group"><label>Категория</label><select v-model="newAchievement.category"><option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option></select></div>
          </div>
          <div class="form-row">
            <div class="form-group"><label>Уровень</label><select v-model="newAchievement.level"><option v-for="lvl in levels" :key="lvl" :value="lvl">{{ lvl }}</option></select></div>
            <div class="form-group"><label>Дата</label><input v-model="newAchievement.date" type="date" /></div>
          </div>
          ...
          <div class="form-group"><label>Навыки (через запятую)</label><input v-model="newAchievement.skills" placeholder="Например: Python, SQL, Публичные выступления" /></div>
    ...
    <div v-if="selectedAchievement" class="modal-overlay" @click.self="selectedAchievement = null">
      <div class="modal modal-sm">
        <div class="modal-header"><h2>Детали достижения</h2><button class="modal-close" @click="selectedAchievement = null">&times;</button></div>
        <div class="modal-body">
          ...
          <div class="detail-row"><span class="detail-label">Источник</span><span class="detail-value">{{ { manual: 'Ручной ввод', moodle: 'Moodle', cdo: 'ЦДО' }[selectedAchievement.source] || selectedAchievement.source }}</span></div>
...
.btn { display: inline-flex; align-items: center; gap: 6px; padding: 9px 18px; border-radius: 10px; font-size: 13px; font-weight: 600; text-decoration: none; border: none; cursor: pointer; transition: all 0.2s; }
...
.btn-outline { background: white; color: #4361ee; border: 1.5px solid #4361ee; } .btn-outline:hover { background: #eef0ff; }
...
.source-badge { font-size: 10px; padding: 2px 8px; border-radius: 4px; font-weight: 600; }
.source-badge.moodle { background: #eef2ff; color: #4361ee; }
.source-badge.cdo { background: #f3e8ff; color: #7c3aed; }
